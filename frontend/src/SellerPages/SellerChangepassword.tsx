...
const SellerChangepassword = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:8080/api/seller/change-password/${sellerId}`, {
        method: 'PUT',
        headers: {
      ...
      <div className="mx-auto flex max-w-7xl gap-6 px-6 py-8">
        ...
        <main className="flex-1 space-y-8">
          <header className="rounded-2xl bg-white px-6 py-6 shadow-sm sm:px-8">
            <div>
              <p className="text-sm font-semibold uppercase tracking-wide text-gray-500">Security</p>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="flex gap-4 pt-4">
                <button
                  type="submit"
                  disabled={loading}
                  className="flex-1 bg-red-500 text-white px-6 py-3 rounded-lg font-medium hover:bg-red-600 transition disabled:bg-gray-400 disabled:cursor-not-allowed"
                >
                  {loading ? 'Changing Password...' : 'Change Password'}
                </button>
                <button
                  type="button"
                  onClick={() => navigate('/sellersetting')}
