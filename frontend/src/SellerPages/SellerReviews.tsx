...
const SellerReviews = () => {
  ...
  useEffect(() => {
    ...
    if (!user || user.role !== 'VENDOR') {
      toast.error('Please login as a vendor to view reviews')
  ...
  const fetchReviews = async (sellerId: number) => {
    setLoading(true)
    try {
      const response = await fetch(`http://localhost:8080/api/reviews/seller/${sellerId}`)
      if (response.ok) {
            ...
            <div className="mt-6 space-y-4">
              {loading ? (
                ...
                <div className="rounded-2xl border border-dashed border-gray-200 py-16 text-center text-sm text-gray-500">
                  {reviews.length === 0
                    ? 'No reviews found. Reviews will appear here once customers submit them for your products.'
